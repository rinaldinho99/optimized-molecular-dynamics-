...
#pragma acc enter data copyin(pos[0:nd*np], vel[0:nd*np], force[0:(nd*np)], acc[0:(nd*np)])
    }
    else
...
#pragma acc exit data delete(pos[0:nd*np], vel[0:nd*np], force[0:nd*np], acc[0:nd*np])
/*
  Report timing.
*/
...
#pragma acc data present(pos[0:nd*np],vel[0:nd*np],f[0:(nd*np)])
#pragma acc parallel loop gang vector_length(128) private(d, d2, rij, i, j, k) firstprivate(np, nd, PI2) reduction(+:pe, ke) 
  for ( k = 0; k < np; k++ )
  {
// Compute the potential energy and forces.
    for ( i = 0; i < nd; i++ )
      f[i+k*nd] = 0.0;
#pragma acc loop vector private(d, d2, rij, i, j, k)  
    for ( j = 0; j < np; j++ )
    {
      if ( k != j )
      {
...
if (k == 3) {
//	printf("pos[%d]: x %f, y %f, z %f. force: x %f, y %f, z %f\n" , k, pos[k*3+0], pos[k*3+1], pos[k*3+2], f[k*3+0], f[k*3+1], f[k*3+2]);
	printf("");
}
...
#pragma acc data present(pos[0:nd*np],vel[0:nd*np],f[0:(nd*np)], acc[0:nd*np])
#pragma acc parallel loop private(i, j) firstprivate(np, nd, rmass, dt)
  for ( j = 0; j < np; j++ )
  {
    for ( i = 0; i < nd; i++ )
    {
